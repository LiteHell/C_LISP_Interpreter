...
static void
yy_symbol_value_print (FILE *yyo, int yytype, YYSTYPE const * const yyvaluep, obj_t *const result)
{
  FILE *yyoutput = yyo;
  YYUSE (yyoutput);
  YYUSE (result);
...
static void
yy_symbol_print (FILE *yyo, int yytype, YYSTYPE const * const yyvaluep, obj_t *const result)
{
  YYFPRINTF (yyo, "%s %s (",
...
static void
yy_reduce_print (yy_state_t *yyssp, YYSTYPE *yyvsp, int yyrule, obj_t *const result)
{
...
static void
yydestruct (const char *yymsg, int yytype, YYSTYPE *yyvaluep, obj_t *const result)
{
  YYUSE (yyvaluep);
  YYUSE (result);
  if (!yymsg)
...
#line 62 "lisp.y"
			...
			*(yyval.list).value = *(obj_t *)&(yyvsp[-1].any);
			if((yyvsp[0].list).type != NIL) {
				(yyval.list).next = malloc(sizeof(obj_t));
				*(yyval.list).next = *(obj_t *)&(yyvsp[0].list);
			} else
